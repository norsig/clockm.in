  ...
  <div class="container">
    <div class="navbar-header">
      <button type="button" class="navbar-toggle collapsed p-x-0" data-toggle="collapse" data-target="#navbar-collapse">
        ...
        <span><?php echo $fullname; ?><span class="caret"></span></span>
      </button>
  ...
  <div class="container-fluid">
        ...
        <div class="col-lg-6 col-lg-offset-3">
            <div class="panel panel-default timeTab text-center">
              ...
              <div class="panel-body">
                  <p>Here are your recorded times, along with controls for you to use!</p>
                    <div class="btn-group" role="group" aria-label="">
                      <button type="button" id="clockin" class="btn btn-success">Clock In</button>
                      <button type="button" id="lunchout" class="btn btn-danger">Lunch Out</button>
                      <button type="button" id="lunchin" class="btn btn-success">Lunch In</button>
                      <button type="button" id="clockout" class="btn btn-danger">Clock Out</button>
                    </div>
              </div>
        <?php
            echo "<div class='table-responsive'>
            <table class='table table-striped table-condensed table-hover' id='timetab'>
            <tr>
   ...
   <?php include('../footer.php'); ?>
	 <script type="text/javascript">
		...
		$("#clockin").click(function() {	                     $.post("http://www.clockm.in/clock.php", {time:"clockin"}, 
			function(response,status){
			});
			$('#timetab').load ('index.php', 'update=true');
		});
		
		$("#lunchin").click(function() {
			$.post("http://www.clockm.in/clock.php", {time:"lunchin"}, 
			function(response,status){
			});
			$('#timetab').load ('index.php', 'update=true');
		});
		...
		$("#lunchout").click(function() {
			$.post("http://www.clockm.in/clock.php", {time:"lunchout"}, 
			function(response,status){
			});
			$('#timetab').load ('index.php', 'update=true');
		});
		...
		$("#clockout").click(function() {
			$.post("http://www.clockm.in/clock.php", {time:"clockout"}, 
			function(response,status){
			});
			$('#timetab').load ('index.php', 'update=true');
